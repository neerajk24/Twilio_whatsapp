...
export const listofUsers = async (req, res) => {
    try {
        ...
        const caseDataArray = result['soap:Envelope']['soap:Body'][0]
        ['CallStoredProcedureResponse'][0]['CallStoredProcedureResult'][0]['CaseData'];
        ...
        res.status(500).json({ error: 'An error occurred while fetching case data' });
    }
};
...
export const sendMessage = async (req, res) => {
    try {
        ...
        const response = await sendWhatsAppMessage(message.receiver_id, message.content);
        ...
        res.status(200).json({ message: `Sending message to twilio successful , SID : ${response}` });
    } catch (error) {
